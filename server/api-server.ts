...
import { securityHeaders, secureErrorMessages } from "../src/config/security-config";
  ...
  async fetch(req): Promise<Response> {
    ...
    try {
      ...
      // Apps endpoints
        ...
        if (method === "GET") {
          const apps = db.query("SELECT * FROM apps ORDER BY created_at DESC").all() as any[];
        ...
        if (method === "POST") {
          ...
          const newApp = db.query("SELECT * FROM apps WHERE id = ?").get(result.lastInsertRowid) as any;
      ...
      // Get single app
        ...
        if (method === "GET") {
          const app = db.query("SELECT * FROM apps WHERE id = ?").get(appId) as any;
          if (!app) {
      ...
      // Dynamic data endpoints (for generated apps to use)  
        ...
        if (method === "GET") {
          const items = db.query("SELECT * FROM dynamic_data WHERE collection = ? ORDER BY created_at DESC").all(collection) as any[];
        ...
        if (method === "POST") {
          ...
          const newItem = db.query("SELECT * FROM dynamic_data WHERE id = ?").get(result.lastInsertRowid) as any;
      ...
      // Handle single item operations
        ...
        if (method === "GET") {
          const item = db.query("SELECT * FROM dynamic_data WHERE collection = ? AND id = ?").get(collection, id) as any;
          if (!item) {
            return createSecureErrorResponse(secureErrorMessages.NOT_FOUND, 404, 'NOT_FOUND', headers);
          }
          return Response.json({ 
        ...
        if (method === "PUT") {
          ...
          const updatedItem = db.query("SELECT * FROM dynamic_data WHERE collection = ? AND id = ?").get(collection, id) as any;
          if (!updatedItem) {
            return createSecureErrorResponse(secureErrorMessages.NOT_FOUND, 404, 'NOT_FOUND', headers);
          }
        ...
        if (method === "DELETE") {
          const result = db.run("DELETE FROM dynamic_data WHERE collection = ? AND id = ?", [collection, id]);
          if (result.changes === 0) {
            return createSecureErrorResponse(secureErrorMessages.NOT_FOUND, 404, 'NOT_FOUND', headers);
          }
          return Response.json({ success: true }, { headers });
      ...
      // Stats endpoint  
        ...
        const headers = (req as any).secureHeaders;
        const appCount = db.query("SELECT COUNT(*) as count FROM apps").get() as any;
        const dataCount = db.query("SELECT COUNT(*) as count FROM dynamic_data").get() as any;
        const collections = db.query("SELECT DISTINCT collection FROM dynamic_data").all() as any[];
