...
// Simple database implementation using Bun's built-in SQLite
export class SimpleBunDatabase {
  private db: Database;
...
export const simpleDB = new SimpleBunDatabase();

// Export individual pieces for compatibility
export const db = simpleDB;
export const queries = new Proxy({}, {
  get(target, prop) {
    return simpleDB.queries[prop as string];
  }
});
export const userHelpers = new Proxy({}, {
  get(target, prop) {
    return simpleDB.userHelpers[prop as string];
  }
});
export const appHelpers = new Proxy({}, {
  get(target, prop) {
    return simpleDB.appHelpers[prop as string];
  }
});
export const appDataHelpers = new Proxy({}, {
  get(target, prop) {
    return simpleDB.appDataHelpers[prop as string];
  }
});
